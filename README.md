...
## 1. Problem Statement
The objective of this project is to build and evaluate a suite of machine learning classification models to accurately diagnose breast cancer. Using features computed from a digitized image of a fine needle aspirate (FNA) of a breast mass, the models predict whether a tumor is malignant (cancerous) or benign (non-cancerous). This dashboard allows for real-time evaluation and comparison of these models to determine the most effective diagnostic tool.
...
* **Logistic Regression:** Showed excellent class separability with an AUC of 0.9977. However, it struggled slightly with false negatives, yielding a lower Recall of 0.9070 compared to the top performers.
* **Decision Tree:** While it achieved a respectable Recall of 0.9302 (tying for the highest), it performed the poorest overall across all other metrics, notably scoring the lowest Accuracy (0.9474), AUC (0.9440), and MCC (0.8880), indicating it is the least robust model of the suite.
...
* **Random Forest:** Proved to be a very strong, well-rounded ensemble model. It tied for the highest Recall (0.9302) while maintaining excellent Accuracy (0.9649) and a high AUC (0.9953), successfully smoothing out the variances seen in the standalone Decision Tree.
...
### Overall Winner
Based on the comprehensive evaluation metrics, **Naive Bayes** is declared the overall winner. 
